...
# The minimum size of a patch to be considered valid.
parser.add_argument('--min_size', '-m', action='store', type=int, default=5)
...
if args.cloudstorage:
    ...
    file_name_prefix = f'forest_status_denoised_{name}/forest_status_denoised_v1_{args.data}'
    image_manifests = {}
assetID = f'projects/{args.project}/assets/forest_status_denoised_{name}/forest_status_denoised_v1_{args.data}'
description_base = f'{name}_ForestStatusDenoised_{args.data}'

forest_status_coll = (
    ee.ImageCollection(
        f'projects/{args.project}/forest_status_{name}/')
...
def denoise_value(image, value, min_size):
    image = image.updateMask(image.eq(value))
    groups = image.connectedPixelCount(10, False)
    too_small = groups.lt(min_size)
    image = (image.And(too_small.Not())
                  .unmask()
                  .multiply(value)
                  .toUint8())
    return image
...
for i in range(gridSize):
    ...
    gridCellBuffer = gridCell.buffer(distance=1000)
    ...
    for year in range(args.start, args.end + 1):
        forest_status = (forest_status_coll
        ...
        # For each potential value, count size of each patch of that value
        # and set to 0 if too small. Summing images together gets denoised image.
        values = ee.List.sequence(0, 7)
        images = ee.ImageCollection(
            values.map(lambda v: denoise_value(forest_status, v, args.min_size)))
        fs_denoised = images.sum()

        fs_denoised = (fs_denoised.set('source', args.data)
