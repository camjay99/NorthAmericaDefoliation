...
if args.cloudstorage:
    ...
    file_name_prefix = f'seasonal_trend_{name}/seasonal_trend_v2_{args.data}'
    image_manifests = {}
    if args.rescale:
        file_name_prefix += '_rescaled'
description_base = f'{name}_Trends_{args.data}'
assetID=f'projects/{args.project}/assets/seasonal_trend_{name}/seasonal_trend_v2_{args.data}'
if args.rescale:
    assetID += '_rescaled'
